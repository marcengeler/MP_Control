 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	for (unsigned int i = 0; i < N; i++) {
		...
		fg[0] += 3000.0 * CppAD::pow(vars[cte_start + i], 2);
		fg[0] += 1500.0 * CppAD::pow(vars[epsi_start + i], 2);
		fg[0] += CppAD::pow(vars[v_start + i] - 100.0, 2);
		
		if (i < N -1) {
			// Minimize actuators
			fg[0] += 10.0 * CppAD::pow(vars[delta_start + i], 2);
			fg[0] += 10.0 * CppAD::pow(vars[a_start + i], 2);
		}
	...
	for (unsigned int i = 0; i < N - 1; i++) {
		...
		fg[2 + psi_start + i] = psi1 - (psi0 - 1/Lf * v0 * delta0 * dt);
		fg[2 + v_start + i] = v1 - (v0 + a0 * dt);
		fg[2 + cte_start + i] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
		fg[2 + epsi_start + i] = epsi1 - ((psi0 - psides0) - 1/Lf * v0 * delta0 * dt);
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  return {solution.x[delta_start], 0.3, solution.x[a_start]};
}
